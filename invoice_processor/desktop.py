...
class ApiClient:
    def __init__(self, base_url="http://127.0.0.1:8000/"):
        ...
        self.invoice_types_url = f"{self.invoices_url}/invoice-types/" # URL base para tipos
    ...
    def get_invoice_types(self):
        ...
        if isinstance(result, dict) and "error" in result:
            print(f"get_invoice_types: Error recibido - {result['error']}")
            return []
        return result if isinstance(result, list) else []
...
class MappingWindow(QWidget):
    ...
    def load_invoice_types(self):
        ...
        print("load_invoice_types: Cargando tipos...")
        ...
        self.invoice_types_data = {} # Limpiar datos cacheados

        types = self.api_client.get_invoice_types()
        if types: # Si la API devolvió una lista (puede estar vacía)
            print(f"load_invoice_types: {len(types)} tipos recibidos.")
            ...
            try:
                ...
                print("load_invoice_types: Advertencia - No se pudo ordenar los tipos por nombre.")
            ...
            for inv_type in types_sorted:
                ...
                if type_id is not None and type_name:
                    ...
                    self.invoice_types_data[type_id] = inv_type
            print(f"load_invoice_types: {len(self.invoice_types_data)} tipos añadidos al ComboBox.")
        else: # Si la API devolvió [] por error o no encontró tipos
            print("load_invoice_types: No se encontraron tipos o hubo un error al cargarlos.")
    ...
    def on_type_selected(self, index):
        ...
        if selected_id is None: # Opción "-- Crear Nuevo Tipo --" seleccionada
            ...
            self.current_invoice_type_id = selected_id
            # Cargar datos del tipo seleccionado
            # Intentar usar los datos cacheados primero
            type_data = self.invoice_types_data.get(selected_id)
            ...
            if type_data and 'extraction_rules' in type_data:
                 ...
                 type_details = self.api_client.get_invoice_type(selected_id)
                 if type_details: # Si get_invoice_type devolvió un diccionario (sin error)
                     self.invoice_types_data[selected_id] = type_details # Actualizar caché con detalles completos
                     self.populate_type_fields(type_details)
                 else: # Si get_invoice_type devolvió None (error)
    ...
    def populate_type_fields(self, type_data):
        """Rellena los campos de entrada con los datos de un InvoiceType."""
    ...
    def save_type(self):
        ...
        result = None
        if self.current_invoice_type_id is None:
            # --- Crear Nuevo Tipo ---
            print(f"save_type: Llamando a create_invoice_type...")
            result = self.api_client.create_invoice_type(payload)
            print(f"save_type: Respuesta de create_invoice_type: {result}")
            ...
            print(f"save_type: Llamando a update_invoice_type para ID: {self.current_invoice_type_id}...")
            result = self.api_client.update_invoice_type(self.current_invoice_type_id, payload)
            print(f"save_type: Respuesta de update_invoice_type: {result}")
            ...
            if isinstance(result, dict) and 'id' in result and "error" not in result:
                 ...
                 self.invoice_types_data[updated_id] = result # Actualizar caché
