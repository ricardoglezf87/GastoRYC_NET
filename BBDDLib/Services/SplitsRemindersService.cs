...
namespace GastosRYC.BBDDLib.Services
{
    public class SplitsRemindersService
    {
        ...
        {
            return RYCContextService.getInstance().BBDD.splitsReminders?.ToList();
        }

        public List<SplitsReminders>? getbyTransactionidNull()
        {
            return RYCContextService.getInstance().BBDD.splitsReminders?.Where(x => x.transactionid == null).ToList();
        }
        ...
        {
            return RYCContextService.getInstance().BBDD.splitsReminders?.FirstOrDefault(x => id.Equals(x.id));
        }
